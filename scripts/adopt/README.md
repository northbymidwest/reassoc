...
- `--method-calls`: rewrite `x.mul(y)` into `(x * y)` and `x.add_assign(y)` into
...
2. `--method-calls`: 938 errors, two causes. `self.mul_assign(x)` in a
   ...
   45 degrees off said so.
3. With both fixes: **0 compile errors; `ir` reports 0 strict float ops
   left; debug 3417/3417; release 3416/3417, where `vec3::test_slerp`
...
any given target, and those are most of "never entered". Trait required
...
impl, inlined strict, which user code cannot annotate) and root finding in
...
so 2015, where `::reassoc::..` needs `extern crate reassoc;` (added).

| crate | what it probes | result |
| --- | --- | --- |
| **wide** 1.6 (SIMD wrapper types, `unsafe` intrinsics, 31 `macro_rules!`, per-arch cfg) | passthrough of SIMD wrappers, arithmetic inside `unsafe`, macro templates | **0 errors; debug 186/186**, and every `f32x4 * f32x4` goes through the blanket; the `ir` and release numbers are in `reassoc-adopt/` |
...
| ultraviolet | 1146 to **2** | four `passthrough!(foreign wide::f32xN)` lines; the 2 left are upstream's own `deny(unused)` |
| fixed, num-bigint, ordered-float, chrono | 4-11 | all generic type parameters |
| tiny-skia | 25 to 14 | after opting in `tiny_skia_path::{Point, f32x2, NormalizedF32}`; 4 are the inference limitation now documented |
...
only lyon_geom names any: 474 to 463. rustfft, nalgebra and palette name
...
- **noise**: types declared in `macro_rules!` templates (`struct $name<T>`)
  never got the derive (201 to 66 errors).
