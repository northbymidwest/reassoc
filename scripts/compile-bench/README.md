...
```bash
scripts/compile-bench.sh                       # 600 fns x ~40 ops, 30 user types
...
Per rewritten operator: ~26us dispatch (was ~21us before the tag, whose whole
cost), ~21us expansion (~9us with optimized macros).
...
Per rewritten operator: ~21us dispatch, ~21us expansion (~12us with optimized
macros). `resolve-crate-name` added ~5us per operator before the crate name
...
that were built and rejected. So ~20us per operator is the floor for this
...
| expanded (dispatch only) | 1.68s | n/a | n/a |
...
scopes, not to project size: ~100k rewritten operators is about +10s on a full
check or build by default, about +5s with optimized macros, about +2s of which
is dispatch.
