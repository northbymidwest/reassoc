...
- Generic functions cannot use `#[algebraic]`. `fn g<T: Mul<Output = T>>(a: T,
  ...
  type to find an impl for. The diagnostic says so.
- Operands of different types are rejected, exactly as they are in plain Rust —
  ...
  alike. See [Diagnostics](#diagnostics) for how closely the errors match.
...
outside it. Mostly it does — the message text, the operand the caret sits on,
