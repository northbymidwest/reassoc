...
//! `.sum()` and `.product()` calls are rewritten as well, and `.powi(n)`
//! under the `powi` feature: into `f32` or `f64` the reductions fold with
//! the algebraic operator and `powi` is algebraic multiplies, on anything
//! else they are the type's own. `#[algebraic(closures = false)]`,
...
//! # #[allow(dead_code)] impl Big { fn powi(self, n: i32) -> Big { Big(Box::new(self.0.powi(n))) } }
//! // .. its `+ - * / %` and `op=` impls, `Sum` / `Product` (and `powi` with that feature) ..
//!
//! #[passthrough]
...
//! `.sum()`), a `powi(self, i32)` method under the `powi` feature (generic
//! code may `.powi(n)` there), and implements one marked trait. What the attribute writes into the
