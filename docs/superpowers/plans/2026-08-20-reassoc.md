...
> **Historical. Finished, superseded, and not a description of this crate.**
>
> This is the task-by-task plan that built the first version, written on
> 2026-08-20 against
> [the original spec](../specs/2026-08-20-reassoc-design.md). Every step in
> it is done, and much of what it specifies was later replaced: the `AlgAdd<B,
> O>` / `alg_add` traits it stamps out gave way in 0.3.0 to `AddRhs<Lhs, O,
> Tag>` keyed on the right operand, the marker-plus-blanket opt-in, and the
> tag parameter that makes `passthrough!(foreign ..)` legal under the orphan
> rule. Code blocks below are the plan's, not the crate's.
>
> Kept as a record of how it was built. Do not implement from it, and do not
> take a conflict between this file and the source as a bug. For what the
> crate does and why each shape is the way it is, see
> [`docs/design.md`](../../design.md), `CLAUDE.md`, and
