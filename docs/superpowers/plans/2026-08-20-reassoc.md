...
**Tech Stack:** Rust 1.98 (MSRV), `no_std` facade, `syn` 2.0 / `quote` / `proc-macro2` for the proc macro, `trybuild` for UI tests.
...
```toml
[dependencies]
syn = { version = "2.0", features = ["full", "visit-mut", "parsing", "printing", "proc-macro"] }
quote = "1.0"
proc-macro2 = "1.0"
```
...
`BinOp` does not expose `.span()` directly on all variants in every syn version; if `binary.op.span()` does not compile, use `syn::spanned::Spanned` — add `use syn::spanned::Spanned;` and call `binary.op.span()` through that trait.
...
Note for the implementer: in syn 2, `a += b` parses as `Expr::Binary` with `BinOp::AddAssign` — there is no separate `ExprAssignOp` node. This was verified against syn 2.0.117.
