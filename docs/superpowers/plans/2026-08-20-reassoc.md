...
- **Macro names:** `alg!`, `#[algebraic]`, `plain!`, `passthrough!`.
...
### Task 4: `passthrough!`, `plain!`, and diagnostics
...
- Produces: `reassoc::passthrough!` with two arms — `passthrough!(Ty)` generating all five same-type impls, and `passthrough!(op: A, B => O)` generating one heterogeneous impl where `op` is one of `add sub mul div rem`. `reassoc::plain!(expr)` as an identity macro.
...
```bash
git add reassoc
git commit -m "feat: passthrough! and plain! macros with pinned diagnostics
...
The `use syn::spanned::Spanned;` import is required: on syn 3, `BinOp::span()` comes from the `Spanned` trait and does not resolve without it. This was verified against syn 3.0.3 — omitting the import fails with `E0599`.
...
### Task 7: `plain!` recognition and macro non-descent
...
- Produces: `plain!(..)` subtrees emitted verbatim; all other macro invocations left untouched.
...
Run: `cargo test -p reassoc --test alg`
Expected: FAIL — `plain` is not stripped, so `alg!(plain!(..))` leaves a `plain!` call that expands *after* the rewriter and defeats the purpose; and/or the `format!` case breaks.
...
```bash
git add reassoc-macros/src/rewrite.rs reassoc/tests/alg.rs
git commit -m "feat: strip plain! subtrees and never descend into other macros
...
//! summation above all — must be wrapped in [`plain!`]:
//!
//! ```
//! # use reassoc::{algebraic, plain};
//! # #[algebraic]
...
//! let y = xs[i] - c;
//! let t = sum + y;
//! c = plain!((t - sum) - y);  // algebraically zero; must not be optimized away
//! sum = t;
//! # }
//! # sum
//! # }
...
- `plain!(expr)` — opt a subexpression back out to strict IEEE.
...
code in `plain!`** — `(t - sum) - y` is algebraically zero, and reassociation
will delete it.
...
- `plain!` is recognized **by name** — any macro whose final path segment is
  `plain`. If you have an unrelated macro called `plain` in scope, `alg!` will
  intercept it and insert its body unexpanded instead of invoking it. A proc
  macro cannot resolve macro identity on stable Rust, so this cannot be
  detected; rename such a macro, or keep it outside a rewritten scope.
