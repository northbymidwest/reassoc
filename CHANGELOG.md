...
- **The machinery lives under `reassoc::__private`.** `ops` and `traits`
  were at the crate root, which read as a surface. They are now
  ...
  API, and a path typed into `__private` by hand looks like what it is. The
  attributes and `alg!` are unchanged, and they were the whole contract, so
  this is a patch. `ops` and the marker are hidden from docs; `traits`
